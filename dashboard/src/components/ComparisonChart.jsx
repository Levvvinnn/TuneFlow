  ...
  return (
    <div style={{ background: "#1e2130", borderRadius: 12, padding: 20, marginBottom: 20 }}>
      ...
      <div style={{ fontSize: 12, color: "#64748b", marginBottom: 14 }}>
        p95 latency across iterations. Lower = better. Faster convergence = more efficient agent strategy.
      </div>
      <ResponsiveContainer width="100%" height={280}>
        <LineChart data={data} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
          ...
          <YAxis stroke="#64748b" label={{ value: "p95 latency (ms)", angle: -90, position: "insideLeft", fill: "#64748b" }} />
          <Tooltip
            contentStyle={{ background: "#1e2130", border: "1px solid #334155", borderRadius: 8 }}
            labelStyle={{ color: "#94a3b8" }}
          />
          <Legend />
          <Line type="monotone" dataKey="a_p95" name={labelA} stroke="#3b82f6" dot={{ r: 4 }} strokeWidth={2.5} />
          <Line type="monotone" dataKey="b_p95" name={labelB} stroke="#f97316" dot={{ r: 4 }} strokeWidth={2.5} strokeDasharray="5 3" />
        </LineChart>
      </ResponsiveContainer>
